...
function analyzeItem(item, prod, settings) {
  const {
    threshold, minRoi, overhead, overheadMin = 0, prepFee = 0,
    priceBasis, pbMap,
  ...
  // True cost = cost + overhead + prep fee
  // Overhead = max(cost × overhead%, overheadMin$). Ensures small-cost items
  // get at least the floor. Prep fee is added on top as a flat per-unit amount.
  const overheadPct = item.cost * overhead;
  const overheadAmount = Math.max(overheadPct, overheadMin || 0);
  const trueCost = Math.round((item.cost + overheadAmount + (prepFee || 0)) * 100) / 100;
  ...
  // Target buy price — inversion of the true-cost formula. Given a target
  // trueCost (from ROI/profit constraint), solve for the supplier cost that
  // would yield it. Two regimes:
  //   • If cost × overhead% > overheadMin: trueCost = cost × (1 + overhead%) + prepFee
  //       → cost = (trueCost - prepFee) / (1 + overhead%)
  //   • Else: trueCost = cost + overheadMin + prepFee
  //       → cost = trueCost - overheadMin - prepFee
  // Pick whichever regime the resulting supplier cost actually falls into.
  function invertTrueCost(targetTrueCost) {
    const candA = (targetTrueCost - (prepFee || 0)) / (1 + overhead);
    const candB = targetTrueCost - (overheadMin || 0) - (prepFee || 0);
    // Test which regime the candidate is consistent with
    const validA = candA * overhead >= (overheadMin || 0) - 0.001;
    const validB = candB * overhead <= (overheadMin || 0) + 0.001;
    if (validA && !validB) return candA;
    if (validB && !validA) return candB;
    // Both/neither valid (edge case near the boundary): pick the more conservative one
  ...
  if (netSale != null) {
    ...
    targetSupplier = Math.round(invertTrueCost(targetTrueCost) * 100) / 100;
...
function notFoundResult(item, overhead, overheadMin = 0, prepFee = 0) {
  const oh = Math.max(item.cost * overhead, overheadMin || 0);
  const tc = Math.round((item.cost + oh + (prepFee || 0)) * 100) / 100;
  return {
    sku: item.sku, upc: item.upc, asin: "", title: item.name || item.upc,
    qtyAvail: item.qtyAvail ?? null,
    cost: item.cost, trueCost: tc,
